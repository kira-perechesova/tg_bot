...
async def get_student_schedule(class_n: str):
    async with async_session() as session:
        stmt = select(StudentSchedule).where(StudentSchedule.class_n == class_n)
        return await session.scalar(stmt)

async def get_teacher_schedule_by_name(teacher_name: str):
    async with async_session() as session:
